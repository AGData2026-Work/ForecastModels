...
An evaluation of two neural architectures (RNN, GRU) for a production forecasting
problem, not a research playground. The protocol is governed by `PROJECT_HANDOFF.md`
sections 5.2 (evaluation) and 5.3 (change control), which the owner has marked as
locked decisions.
...
**As of D-36 (2026-09-16), the only benchmark for these models is the naive
(do-nothing) forecast.** The panel fixed-effects model (`panel_fe`) is a separate
workstream managed internally by the owner; do not compare these models against it,
do not report `vs_panel_fe_pct` as a headline figure, and do not treat
`change_control`'s 5%-vs-incumbent gate as the pass/fail criterion for this
workstream going forward. This replaces the "challenger vs incumbent" framing this
file used before D-36 -- see `docs/DECISIONS.md` D-36 for the owner's instruction and
full rationale. The underlying data and code that recompute `panel_fe` on the same
pairs are left in place (nothing was deleted, D-16's own precedent), since dropping
...
**Always score the naive benchmark on the same pairs as the challenger.** Never
compare an MAE from one grid to an MAE from another. `metrics.paired_table` does
this correctly; do not bypass it. (Before D-36 this rule was phrased around the
incumbent specifically; the principle -- never let the challenger and its benchmark
be computed on different pairs -- is unchanged, only which benchmark it applies to.)
...
**Superseded by D-36, kept for history:** this rule used to read "Read
`change_control.json` from the conditional runs -- the incumbent appears to have
been measured with drivers known in advance, so only the conditional arm is
comparable to it." That comparability question no longer matters for this
workstream's headline reporting, since the incumbent is out of scope entirely. The
conditional/unconditional distinction itself still matters for the next rule
(foreknowledge is foreknowledge regardless of what it's being compared against), so
`docs/CONDITIONAL_CONVENTION.md` is still worth reading for that reason.
...
- `vs_naive_pct`: **positive means the challenger is better.** This is the only
  benchmark comparison reported as of D-36; `vs_panel_fe_pct` still exists in
  `paired_table`'s output (nothing was removed from the code) but is out of scope
  for this workstream's headline reporting -- see "What this is" above.
...
## Layout
    ...
    src/metrics.py         MAE/MAPE/direction, paired tables, Diebold-Mariano, the
                           gate (defined against the incumbent; not this workstream's
                           pass/fail criterion as of D-36, see "What this is" above)
...
4. **`config.yaml` versus outputs.** Owner decision D-25 of 2026-08-12 made panel FE
   ...
   production. Panel FE was this project's incumbent; as of D-36 (2026-09-16) this
   workstream no longer compares against any incumbent, panel FE included, so this
   is historical context rather than a live comparison target. Do not reintroduce
   C8 comparisons, and do not reintroduce panel FE comparisons either without being
   asked.
...
then whether the naive benchmark was recomputed on the same pairs, then seed spread.
